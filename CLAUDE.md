...
- **YouTube音源の取得は`yt-dlp`(`uv`で作った専用venvにインストール)を使う**(`src/introquiz/youtubeResolver.ts`)。以前は`play-dl`を使っていたが、`play-dl`は実質メンテナンス終了(2023年9月公開の`1.9.7`が最新かつ最終版)しており、YouTube側の仕様変更で`TypeError: Invalid URL`と共に壊れた。`yt-dlp`はPython製だが、当初はPython本体をイメージに入れずに済む公式スタンドアロンLinuxバイナリ(PyInstaller onefile形式)を`Dockerfile`の実行ステージで`ADD`により直接取得していた。しかし後述の起動オーバーヘッド問題が判明したため、`uv`で作成したvenvにインストールする方式に切り替えた(`youtube-dl-exec`はシステムPython必須、`yt-dlp-wrap`/`yt-dlp-exec`は非サポート/archivedのため、npmラッパーはいずれも不採用)。
  ...
  - **venv切り替え後も本番の`buffering`→`playing`は約11〜12秒とほぼ改善しておらず(切替前の最良値約10.7秒と同水準)**。`spawn()`呼び出しと、Nodeの`child.on("spawn")`イベント(OSがプロセス起動を完了した時点)の両方にログを追加して切り分けた結果(`ytdlpSpawnRequested`/`ytdlpProcessSpawned`)、**Node/OS側のfork/exec自体は約0.35秒と高速で、ボトルネックではないと判明**。一方、OSがプロセス起動を完了してからyt-dlpが最初のデバッグ行を出力するまでに約6.6秒かかっており、遅延は完全にプロセス内部(Pythonの起動処理)で発生している。さらに、内容的にはほぼ無コストなはずの連続した2つの静的デバッグ行の間にも約1.1秒の間隔があり、通常のimport/実行コストでは説明しづらい。ローカルでの`yt-dlp --version`実測(初回361ms、以降113〜115ms)とは桁が違うことも踏まえ、**プロセスは起動しているがCPU実行時間を十分にもらえていない(Northflankの`nf-compute-20`開発用サンドボックスにおけるcgroup CPUスロットリング)可能性が高いと見ている**。アプリ側のログでの切り分けはここが限界で、次の確認はNorthflankダッシュボードで該当時刻のCPU使用率/スロットリングのメトリクスを直接見ること。スロットリングが確認された場合、対策はアプリ側の工夫ではなくcompute planの引き上げ(コスト増とのトレードオフ)が必要になる。
  - **実際にNorthflankダッシュボードでCPUメトリクスを確認したところ、該当リクエストの時刻に一致してCPU使用率が0.19 vCPU(95.4%、`nf-compute-20`の割り当てにほぼ張り付く水準)までスパイクしていることを確認し、CPUスロットリングが根本原因である可能性が高いと判断した**。yt-dlpのPython起動コストは1プロセスあたりほぼ固定費であり、アプリ側でこれ以上大きく削れる余地は薄い(常駐プロセス化のような大掛かりなアーキテクチャ変更をしない限り)。compute planの引き上げが最も直接的な解決策だが、コスト増を伴うインフラ側の判断のため、2026年08月時点では対応を保留し現状維持としている。再度この問題に着手する場合は、まずcompute planの引き上げを検討するか、常駐ワーカープロセス化のような設計変更を検討する。
