...
- **YouTube音源の取得は`yt-dlp`(`uv`で作った専用venvにインストール)を使う**(`src/introquiz/youtubeResolver.ts`)。以前は`play-dl`を使っていたが、`play-dl`は実質メンテナンス終了(2023年9月公開の`1.9.7`が最新かつ最終版)しており、YouTube側の仕様変更で`TypeError: Invalid URL`と共に壊れた。`yt-dlp`はPython製だが、当初はPython本体をイメージに入れずに済む公式スタンドアロンLinuxバイナリ(PyInstaller onefile形式)を`Dockerfile`の実行ステージで`ADD`により直接取得していた。しかし後述の起動オーバーヘッド問題が判明したため、`uv`で作成したvenvにインストールする方式に切り替えた(`youtube-dl-exec`はシステムPython必須、`yt-dlp-wrap`/`yt-dlp-exec`は非サポート/archivedのため、npmラッパーはいずれも不採用)。
  ...
  - **venv切り替え後も本番の`buffering`→`playing`は約11.2秒とほぼ改善しておらず(切替前の最良値約10.7秒と同水準)、追加調査中**。本番ログでは、`spawn()`呼び出し(≒`AudioPlayer`が`buffering`になる瞬間、`playQuestion`内で`createYoutubeAudioResource`→`audioPlayer.play()`がawaitなしで連続実行されるため両者はほぼ同時刻)からyt-dlpの最初のデバッグ行が出るまで約5.9秒かかっている。さらに、内容的にはほぼ無コストなはずの連続した2つの静的デバッグ行の間にも約1.3秒の間隔があり、単純なimportコストでは説明しづらい。ローカルでの`yt-dlp --version`実測(初回361ms、以降113〜115ms)とは桁が違うため、**Northflankの`nf-compute-20`(開発用サンドボックス)側のCPUスロットリングが支配的な可能性を疑っている**。切り分けのため、`spawn()`呼び出し直後と、Nodeの`child.on("spawn")`イベント(OSがプロセス起動を完了した時点)の両方にログを追加した(`ytdlpSpawnRequested`/`ytdlpProcessSpawned`)。この2つの間隔が大きければNode/OS側のfork/exec自体が遅い(スロットリングの影響を強く受けている)、小さければプロセス内部(Pythonの実行そのもの)が遅いと切り分けられる。あわせて、Northflank側のCPU使用率/スロットリングのメトリクスをダッシュボードで直接確認するのも有効な手段(アプリ側のログだけでは限界がある)。
